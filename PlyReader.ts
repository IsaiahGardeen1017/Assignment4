...
export function getPlyPoints(fileName:string):vec4[]{
    ...
    for(let i:number = 2; i < (numVerts * 6) + 2; i += 6) {
        gg.addVertex(parseFloat(numbers[i]), parseFloat(numbers[i + 1]), parseFloat(numbers[i + 2]), new vec4(parseFloat(numbers[i + 3]), parseFloat(numbers[i + 4]), parseFloat(numbers[i + 5]), 1));//Adds each vertex and normal
    }
